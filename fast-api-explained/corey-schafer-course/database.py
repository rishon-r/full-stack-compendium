...
SQLALCHEMY_DATABASE_URL = "sqlite+aiosqlite:///./blog.db" # Tells SQLAlchemy where to connect, when we want to switch databases, this is the main thing we must aim to change
...
AsyncSessionLocal = async_sessionmaker(
    engine,
    class_=AsyncSession,
    expire_on_commit=False # recommended for async as it prevents issues with expired objects after a commit
...
async def get_db(): # This is a dependency function that provides sessions to our routes. FastAPI's dependency injection will call this function for each request
    async with AsyncSessionLocal() as session: # With makes the session work in a manner similar to a context manager, taking care of cleanup and error handling
        ...
        yield session # In Python, the yield statement is used to turn a standard function into a generator
        ...
        # yield session does three things:
